...
forecast_baseline <- forecast(fit_baseline, h = 50)
forecast_down <- forecast(fit_down, h = 50)
forecast_up <- forecast(fit_up, h = 50)
...
# Decision Criteria
if(t_test_down$p.value < 0.05) {
  print("Decision: p < 0.05 - Conditioning had a measurable effect on H-reflex (Reject H₀ for Down-conditioning)")
} else {
  print("Decision: p >= 0.05 - No measurable effect of conditioning on H-reflex for Down-conditioning")
}

if(t_test_up$p.value < 0.05) {
  print("Decision: p < 0.05 - Conditioning had a measurable effect on H-reflex (Reject H₀ for Up-conditioning)")
} else {
  print("Decision: p >= 0.05 - No measurable effect of conditioning on H-reflex for Up-conditioning")
}
